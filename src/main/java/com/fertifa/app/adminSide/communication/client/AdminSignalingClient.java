package com.fertifa.app.adminSide.communication.client;

import com.fertifa.app.adminSide.communication.AdminIncomingCallServlet;
...
public class AdminSignalingClient implements SignalEventListener {
    private static AdminSignalingClient instance = null;
    private static final String TAG = AdminSignalingClient.class.getName();
    public static String UUID = "ADMIN ID";
    private HttpServletRequest request;
    private HttpServletResponse response;
    private SignalingConnection signalingConnection;
    private static AtomicBoolean isRunning = new AtomicBoolean(false);

    public static synchronized AdminSignalingClient getInstance(SignalingConnection signalingConnection) {
        if (instance == null) {
            instance = new AdminSignalingClient(signalingConnection);
        }
        return instance;
    }
    ...
    private AdminSignalingClient(SignalingConnection signalingConnection) {
    ...
    public AdminSignalingClient addReqRes(HttpServletRequest request, HttpServletResponse response) {
    ...
    public void start() {
        ...
        if (!isRunning()) {
            ...
            signalingConnection.connect(com.fertifa.app.users.communication.constances.UserVideoConstance.SOCKET_URL_DEVELOPMENT);
            isRunning.set(true);
        }
    }
    ...
    public void onMessageReceived(Object message) {
        ...
        if (signalRoom.getEventType().equals(SignalRoom.Event.INCOMING_CALL.name())) {
            // need show incoming call modal

            try {
                AdminIncomingCallServlet adminIncomingCallServlet = new AdminIncomingCallServlet();
                adminIncomingCallServlet.doGet(request, response);
            } catch (Exception e) {
                e.printStackTrace();
            }
