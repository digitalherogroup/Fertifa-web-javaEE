...
public class EnquiryFormNotification implements NotificationManager {

    @Override
    public void sendMessage(Users users, int i) throws IOException, ServletException {
        String message = new AdminMessagesManager().getRightMessage(MessagesEnum.enquiry,users,i);
        String subject = new AdminSubjectManager().getRightMessage(MessagesEnum.enquiry,i);
        int adminMasterCount = new AdminMasterEmailInChargeManager().getRightMessage(MessagesEnum.enquiry);
